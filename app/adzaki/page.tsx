  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12" dir={dir}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-10">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6 text-center">
          <p className="text-4xl font-bold text-[#e94560]">{totalOrders}</p>
            ...
            {zones.map((zone) => (
              <div key={zone.id} className="bg-gray-50 rounded-xl p-4">
                ...
                <div className="flex items-center gap-2">
                  <input
                    type="number"
                    value={zoneRates[zone.id] ?? zone.rate}
                    onChange={(e) =>
                      setZoneRates((prev) => ({ ...prev, [zone.id]: parseInt(e.target.value) || 0 }))
                    }
      ...
      {editingProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto animate-scale-in">
            <div className="p-6 border-b border-gray-100 flex items-center justify-between">
              <h3 className="text-xl font-bold text-gray-900">{t("admin.products.edit", lang)}</h3>
              <button onClick={cancelEdit} className="text-gray-400 hover:text-gray-600 text-2xl leading-none">&times;</button>
            </div>
            <div className="p-6 space-y-4">
              <div className="flex gap-4 mb-4">
                <img src={editingProduct.image} alt="" className="w-20 h-20 object-cover rounded-xl" />
                <div className="flex-1">
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t("admin.products.image", lang)}</label>
                  <input value={editForm.image || ""} onChange={(e) => setEditForm({ ...editForm, image: e.target.value })}
                    className="w-full px-3 py-2 rounded-lg border border-gray-200 focus:border-[#e94560] focus:ring-2 focus:ring-[#e94560]/20 outline-none transition-all text-sm" />
                </div>
              </div>
              <div>
            ...
            <div className="p-6 border-t border-gray-100 flex justify-end gap-3">
              ...
              <button onClick={handleSave} disabled={saving}
                ...
                {saving ? t("admin.saving", lang) : t("admin.save", lang)}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        <div className="p-6 border-b border-gray-100">
          <h2 className="text-xl font-bold text-gray-900">{t("admin.orders", lang)}</h2>
        </div>
        {orders.length === 0 ? (
