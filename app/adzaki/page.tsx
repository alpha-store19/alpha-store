        ...
        <div className="relative">
          ...
          <div className="relative glass rounded-3xl p-10 w-full max-w-sm cyber-border animate-scale-in">
            ...
            <div className="space-y-4">
              ...
              <button onClick={() => setAuthed(password === "alpha123")} className="btn-cyber-solid w-full py-3 text-sm tracking-wider">UNLOCK</button>
...
function AdminDashboard({ tab, setTab, lang, dir }: { tab: string; setTab: (t: string) => void; lang: Lang; dir: string }) {
  ...
  return (
    <div className="min-h-screen bg-dark flex" dir={dir}>
      ...
      {/* Sidebar */}
      <div className={`${sidebarCollapsed ? "w-20" : "w-64"} glass border-r border-white/[0.04] flex flex-col transition-all duration-300 flex-shrink-0`}>
        {/* Logo */}
        ...
        <nav className="flex-1 p-3 space-y-1">
          {sidebarTabs.map(t => (
            <button key={t.key} onClick={() => { setTab(t.key); if (t.key === "orders") setNewOrderCount(0) }}
              ...
              {!sidebarCollapsed && t.key === "orders" && pendingOrders.length > 0 && (
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-hidden">
        ...
        <div className="glass border-b border-white/[0.04]">
          <div className="flex items-center justify-between px-6 h-16">
            <div>
              <h1 className="text-lg font-bold text-white capitalize">{getTabLabel(sidebarTabs.find(t => t.key === tab) || sidebarTabs[0])}</h1>
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => { setNewOrderCount(0); setTab("orders") }}
                className="relative w-10 h-10 rounded-xl bg-white/[0.03] border border-white/[0.06] flex items-center justify-center hover:bg-white/[0.06] transition-all">
                <svg className="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                </svg>
                {newOrderCount > 0 && (
                  <span className="absolute -top-1 -right-1 w-5 h-5 bg-cyber text-dark text-[10px] rounded-full flex items-center justify-center font-bold animate-scale-in">{newOrderCount}</span>
                )}
              </button>
              {/* Refresh */}
              <button onClick={triggerRefresh}
                className="w-10 h-10 rounded-xl bg-white/[0.03] border border-white/[0.06] flex items-center justify-center hover:bg-white/[0.06] transition-all">
                <svg className="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
